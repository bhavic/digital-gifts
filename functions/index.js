...
import stars from '../data/stars.json' assert { type: 'json' };
...
export const shopifyOrderPaid = onRequest(async (req, res) => {
    ...
    try {
        ...
        const slug = buildOrderSlug(orderId, star.id);
        const certificatePageUrl = `https://${slug}.${rootDomain}`;
        const arUrl = `https://${rootDomain}/view?id=${encodeURIComponent(uniqueId)}`;
        ...
        res.status(200).json({ ok: true, uniqueId, slug, certificatePageUrl, arUrl });
    } catch (error) {
        console.error(error);
