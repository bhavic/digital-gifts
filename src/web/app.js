...
// Direct Shopify Storefront API Call (Bypassing SDK for 2026 compatibility)
async function createCart(plan, recipient, frameChecked) {
    const query = `
    ...
    const lines = [
        {
            merchandiseId: plan.variantId,
            quantity: 1,
            attributes: [
                { key: "_Recipient_Name", value: recipient },
                { key: "_Star_Name", value: "Asteria Prime" },
    ...
    if (frameChecked) {
        lines.push({
            merchandiseId: frameAddon.variantId,
            quantity: 1
        });
    }
    ...
    try {
        const response = await fetch('https://4icfjq-74.myshopify.com/api/2025-01/graphql.json', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Shopify-Storefront-Access-Token': 'a7d907b446eab147330ecd0afb875470'
            },
        ...
        // FORCE the internal domain to prevent "Page Not Found" (404)
        // Shopify returns 'auragifts.online/cart/...' but that domain points to Firebase!
        const generatedUrl = new URL(result.data.cartCreate.cart.checkoutUrl);
        generatedUrl.hostname = '4icfjq-74.myshopify.com';
...
async function buyNow() {
    ...
    buyStatus.textContent = "Creating Secure Cart...";
    buyStatus.classList.remove("text-red-400");

    try {
        const checkoutUrl = await createCart(plan, recipient, frameAddonCheckbox.checked);
        buyStatus.textContent = "Redirecting to Payment...";
        window.location.href = checkoutUrl;
    } catch (error) {
        console.error("Shopify Error:", error);
        buyStatus.textContent = "Shopify Connection Error: " + error.message;
        buyStatus.classList.add("text-red-400");
    }
}
